...
//cvTask回调函数(x, y, z)=>(距离, 速度, 偏航角)
void cvTaskCallback(const geometry_msgs::Vector3::ConstPtr& msg);
...
{
    //发送串口
    short distance = (short)(msg->x);
    short velocity = (short)(msg->y);
    short yaw = (short)(msg->z);
    ...
    //distance
    usartBuffer[5] = distance >> 8;
    usartBuffer[4] = distance - (usartBuffer[5] << 8);
    //velocity
    usartBuffer[7] = velocity >> 8;
    usartBuffer[6] = velocity - (usartBuffer[7] << 8);
    //yaw
    usartBuffer[9] = yaw >> 8;
    usartBuffer[8] = yaw - (usartBuffer[9] << 8);
